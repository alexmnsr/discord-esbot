...
from utils.neccessary import is_counting, remove_role, send_embed, add_role
...
class PunishmentsHandler:
    ...
    async def give_text_mute(self, *, user, guild, moderator, reason, duration):
        ...
        guild, member = await add_role(self.client, user_id, guild.id, action_id)
        if not guild:
            return
        ...
        log_embed.add_field(name='Время истечения', value=f'<t:{int((datetime.datetime.now() + datetime.timedelta(seconds=duration)).timestamp())}:R>')
    ...
    async def remove_mute(self, user_id, guild_id):
        if not (mute := await self.database.get_text_mute(user_id=user_id, guild_id=guild_id)):
            return False

        if not await self.database.remove_text_mute(user_id, guild_id):
            return False

        guild, member = await remove_role(self.client, user_id, guild_id, mute['action_id'], 'Mute » Text')
        if guild:
            embed = nextcord.Embed(
                title='Снят текстовый мут',
                description=f'У пользователя {member.mention} снят текстовый мут.',
                color=0x00FF00
            )
            await send_embed(member, embed)

        log_embed = nextcord.Embed(title='Снятие текстового мута', color=0x00FF00)
        log_embed.set_author(name=member.display_name, icon_url=member.display_avatar.url)
